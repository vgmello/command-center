...
import { toClusterDto } from '$lib/server/api/v1/dto';
import { readClusters } from '$lib/server/platform/service';
 ...
 *   get:
 *     summary: List clusters by load
 *     description: >-
 *       Busiest first, already ranked by the source.
...
export const GET: RequestHandler = async ({ url, request }) =>
	apiResponse(request, async () => {
		...
		return { data: (await readClusters(scope, limit)).map(toClusterDto) };
	});
