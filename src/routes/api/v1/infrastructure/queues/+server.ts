...
import { toQueueDto } from '$lib/server/api/v1/dto';
import { readQueues } from '$lib/server/platform/service';

const limitSchema = v.pipe(v.number(), v.integer(), v.minValue(1), v.maxValue(100));

/**
 * @swagger
 * /api/v1/infrastructure/queues:
 *   get:
 *     summary: List message queues
 *     description: >-
 *       With consumer lag, which is the reading that matters on its own.
...
export const GET: RequestHandler = async ({ url, request }) =>
	apiResponse(request, async () => {
		...
		return { data: (await readQueues(scope, limit)).map(toQueueDto) };
	});
