...
import { toServiceMetricsDto } from '$lib/server/api/v1/dto';
import { readServiceMetricSeries } from '$lib/server/platform/service';

/**
 * @swagger
 * /api/v1/services/{slug}/metrics:
 *   get:
 *     summary: Get a service’s metric series
 *     description: >-
 *       Request rate, latency, error rate, saturation, and the split by endpoint and by
 *       instance — all over the same window, so two of them can be compared.
...
export const GET: RequestHandler = async ({ url, params, request }) =>
	apiResponse(request, async () => {
		...
		const rows = await readServiceMetricSeries(scope, slug);

		if (rows === null) throw new NotFoundError(`service with id "${slug}"`);
		return toServiceMetricsDto(rows);
	});
