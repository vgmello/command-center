...
<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6">
	{#each stats as stat (stat.id)}
		...
		<article class="rounded-xl border border-border bg-card px-4 py-3">
			...
			<!--
				One switch on `kind`. The union is what makes this exhaustive: a new tile
				shape is a type error here rather than a blank card in production.
			-->
			{#if stat.kind === 'ratio'}
				<p class="tabular mt-2 text-[26px] leading-none font-semibold {tone?.text ?? ''}">
					{stat.value}<span class="ml-1 text-[15px] text-muted-foreground">
						/ {stat.total}
					</span>
