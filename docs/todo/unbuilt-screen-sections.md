...
| Screen         | Tabs | Built                                                                           |
| -------------- | ---: | ------------------------------------------------------------------------------- |
| Domain detail  |    8 | `overview`, `dependencies`, `services`, `deployments`, `slos`, `infrastructure` |
| Service detail |    8 | `overview`, `metrics`                                                           |
| Infrastructure |    9 | `overview`                                                                      |

Domain detail's remaining two — `alerts` and `logs` — each want their own spec before a task
builds them: `alerts` needs an incidents-scoped read this codebase does not have yet, and
`logs` wants a log source no provider declares. Neither is "add a route" the way
`services`/`deployments`/`slos`/`infrastructure` were once the capability underneath each
already existed — both are blocked on a capability question first.
...
16 of 25 sub-destinations are placeholders, so the product is still a good deal smaller than
